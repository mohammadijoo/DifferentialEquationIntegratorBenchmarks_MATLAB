# Sum-of-Exponentials Memory Acceleration

## Category

Fast approximation for fractional-memory kernels..
...
```text
Not implemented yet. Suggested future file: src/methods/sum_of_exponentials_memory_acceleration.m
```
...
Fractional differential equations include nonlocal memory terms such as a Caputo derivative

$$
{}^C D_t^\alpha y(t)=f(t,y(t)), \qquad 0<\alpha<1.
$$

The main numerical challenge is that the current derivative depends on the full or compressed history of the solution.

### Core idea

Sum-of-Exponentials Memory Acceleration should be understood as part of the **Fast approximation for fractional-memory kernels.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Fast approximation for fractional-memory kernels..
