...
Finite-difference approximation for fractional derivatives..
...
```text
Not implemented yet. Suggested future file: src/methods/grunwald_letnikov.m
```
...
Grünwald-Letnikov Method should be understood as part of the **Finite-difference approximation for fractional derivatives.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Finite-difference approximation for fractional derivatives..
