...
Higher-order scheme for Caputo fractional derivatives..
...
```text
Not implemented yet. Suggested future file: src/methods/l21_sigma_scheme.m
```
...
L2-1σ Scheme should be understood as part of the **Higher-order scheme for Caputo fractional derivatives.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Higher-order scheme for Caputo fractional derivatives..
