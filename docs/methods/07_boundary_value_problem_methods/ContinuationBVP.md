# Continuation for BVPs

## Category

Parameter continuation for nonlinear BVPs..
...
```text
Not implemented yet. Suggested future file: src/methods/continuation_bvp.m
```
...
Continuation for BVPs should be understood as part of the **Parameter continuation for nonlinear BVPs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Parameter continuation for nonlinear BVPs..
