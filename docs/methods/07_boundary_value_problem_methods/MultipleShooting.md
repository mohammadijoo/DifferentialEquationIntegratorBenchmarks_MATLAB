...
BVP method using several shooting intervals..
...
```text
Not implemented yet. Suggested future file: src/methods/multiple_shooting.m
```
...
Multiple Shooting should be understood as part of the **BVP method using several shooting intervals.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into BVP method using several shooting intervals..
