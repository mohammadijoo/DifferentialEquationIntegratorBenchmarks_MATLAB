...
BVP method converting unknown initial conditions to a root solve..
...
```text
Not implemented yet. Suggested future file: src/methods/single_shooting.m
```
...
Single Shooting should be understood as part of the **BVP method converting unknown initial conditions to a root solve.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into BVP method converting unknown initial conditions to a root solve..
