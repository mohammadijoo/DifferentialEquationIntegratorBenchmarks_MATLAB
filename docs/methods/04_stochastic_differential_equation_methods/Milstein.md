...
Strong order-one method for suitable SDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/milstein.m
```
...
Milstein Method should be understood as part of the **Strong order-one method for suitable SDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Strong order-one method for suitable SDEs..
