...
SDE method targeting weak/statistical accuracy..
...
```text
Not implemented yet. Suggested future file: src/methods/weak_taylor_method.m
```
...
Weak Taylor Method should be understood as part of the **SDE method targeting weak/statistical accuracy.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into SDE method targeting weak/statistical accuracy..
