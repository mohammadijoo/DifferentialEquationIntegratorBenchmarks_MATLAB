...
Implicit SDE method for stiff stochastic systems..
...
```text
Not implemented yet. Suggested future file: src/methods/implicit_euler_maruyama.m
```
...
Implicit Euler-Maruyama should be understood as part of the **Implicit SDE method for stiff stochastic systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Implicit SDE method for stiff stochastic systems..
