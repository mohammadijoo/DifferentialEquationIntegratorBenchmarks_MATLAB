...
Euler variant for SDEs with superlinear drift..
...
```text
Not implemented yet. Suggested future file: src/methods/tamed_euler.m
```
...
Tamed Euler should be understood as part of the **Euler variant for SDEs with superlinear drift.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Euler variant for SDEs with superlinear drift..
