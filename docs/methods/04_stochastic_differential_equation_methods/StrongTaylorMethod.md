...
SDE method targeting pathwise strong accuracy..
...
```text
Not implemented yet. Suggested future file: src/methods/strong_taylor_method.m
```
...
Strong Taylor Method should be understood as part of the **SDE method targeting pathwise strong accuracy.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into SDE method targeting pathwise strong accuracy..
