...
Stable split-step method for stiff SDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/split_step_backward_euler.m
```
...
Split-Step Backward Euler should be understood as part of the **Stable split-step method for stiff SDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Stable split-step method for stiff SDEs..
