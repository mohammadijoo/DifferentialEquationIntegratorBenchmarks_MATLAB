...
Runge-Kutta-type methods for SDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/stochastic_runge_kutta.m
```
...
Stochastic Runge-Kutta should be understood as part of the **Runge-Kutta-type methods for SDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Runge-Kutta-type methods for SDEs..
