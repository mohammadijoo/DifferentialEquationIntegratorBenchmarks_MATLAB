...
Basic strong method for stochastic differential equations..
...
```text
Not implemented yet. Suggested future file: src/methods/euler_maruyama.m
```
...
Euler-Maruyama should be understood as part of the **Basic strong method for stochastic differential equations.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Basic strong method for stochastic differential equations..
