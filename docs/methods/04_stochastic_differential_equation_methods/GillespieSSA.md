...
Exact stochastic simulation algorithm for reaction networks..
...
```text
Not implemented yet. Suggested future file: src/methods/gillespie_ssa.m
```
...
Gillespie SSA should be understood as part of the **Exact stochastic simulation algorithm for reaction networks.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Exact stochastic simulation algorithm for reaction networks..
