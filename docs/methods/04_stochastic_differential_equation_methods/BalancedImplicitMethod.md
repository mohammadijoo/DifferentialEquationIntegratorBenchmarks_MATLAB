...
Implicit SDE method balancing drift and noise effects..
...
```text
Not implemented yet. Suggested future file: src/methods/balanced_implicit_method.m
```
...
Balanced Implicit Method should be understood as part of the **Implicit SDE method balancing drift and noise effects.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Implicit SDE method balancing drift and noise effects..
