...
Direct solver for block tridiagonal systems..
...
```text
Not implemented yet. Suggested future file: src/methods/block_tridiagonal_solver.m
```
...
Block Tridiagonal Solver should be understood as part of the **Direct solver for block tridiagonal systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Direct solver for block tridiagonal systems..
