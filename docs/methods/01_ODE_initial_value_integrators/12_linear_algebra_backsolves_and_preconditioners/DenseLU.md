...
Direct dense linear solve for implicit methods..
...
```text
Not implemented yet. Suggested future file: src/methods/dense_lu.m
```
...
Dense LU Factorization should be understood as part of the **Direct dense linear solve for implicit methods.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Direct dense linear solve for implicit methods..
