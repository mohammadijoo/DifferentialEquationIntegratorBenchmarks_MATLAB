...
Multilevel preconditioner/solver for large sparse systems..
...
```text
Not implemented yet. Suggested future file: src/methods/algebraic_multigrid.m
```
...
Algebraic Multigrid should be understood as part of the **Multilevel preconditioner/solver for large sparse systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Multilevel preconditioner/solver for large sparse systems..
