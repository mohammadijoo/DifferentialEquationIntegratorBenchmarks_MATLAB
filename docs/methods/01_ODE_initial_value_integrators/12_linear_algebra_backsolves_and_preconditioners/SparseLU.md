...
Sparse direct linear solve for stiff systems..
...
```text
Not implemented yet. Suggested future file: src/methods/sparse_lu.m
```
...
Sparse LU Factorization should be understood as part of the **Sparse direct linear solve for stiff systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Sparse direct linear solve for stiff systems..
