...
Krylov solver for nonsymmetric sparse systems..
...
```text
Not implemented yet. Suggested future file: src/methods/bi_cgstab.m
```
...
BiCGSTAB should be understood as part of the **Krylov solver for nonsymmetric sparse systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Krylov solver for nonsymmetric sparse systems..
