...
Stationary linear iteration using immediate updates..
...
```text
Not implemented yet. Suggested future file: src/methods/gauss_seidel.m
```
...
Gauss-Seidel should be understood as part of the **Stationary linear iteration using immediate updates.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Stationary linear iteration using immediate updates..
