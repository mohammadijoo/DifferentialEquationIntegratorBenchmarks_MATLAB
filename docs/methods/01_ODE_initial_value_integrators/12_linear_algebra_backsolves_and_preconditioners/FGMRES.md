...
```text
Not implemented yet. Suggested future file: src/methods/fgmres.m
```
...
Implicit integrators repeatedly require linear systems such as

$$
A\Delta x=b,
$$

where $A$ may be dense, sparse, banded, symmetric, nonsymmetric, or only available through matrix-vector products. The selected backsolve or iterative linear solver strongly affects CPU time, robustness, and memory use.

### Core idea

FGMRES should be understood as part of the **Flexible GMRES for variable preconditioning.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Flexible GMRES for variable preconditioning..
...
implicit methods where linear-solve cost dominates runtime
