...
Stable factorization for least-squares and constrained solves..
...
```text
Not implemented yet. Suggested future file: src/methods/qrfactorization.m
```
...
QR Factorization should be understood as part of the **Stable factorization for least-squares and constrained solves.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Stable factorization for least-squares and constrained solves..
