# Jacobian-Free Matrix-Vector Products

## Category

Finite-difference directional derivatives for matrix-free Newton-Krylov..
...
```text
Not implemented yet. Suggested future file: src/methods/jacobian_free_matvec.m
```
...
a one-step or multistep method computes approximations $y_n\approx y(t_n)$ on a grid $t_n=t_0+nh$.

### Core idea

Jacobian-Free Matrix-Vector Products should be understood as part of the **Finite-difference directional derivatives for matrix-free Newton-Krylov.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Finite-difference directional derivatives for matrix-free Newton-Krylov..
