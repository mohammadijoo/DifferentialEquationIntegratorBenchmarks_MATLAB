...
Krylov solver for nonsymmetric linear systems..
...
```text
Not implemented yet. Suggested future file: src/methods/gmres.m
```
...
GMRES should be understood as part of the **Krylov solver for nonsymmetric linear systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Krylov solver for nonsymmetric linear systems..
