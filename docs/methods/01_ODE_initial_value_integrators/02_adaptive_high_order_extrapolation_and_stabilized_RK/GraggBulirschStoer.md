...
Modified midpoint plus extrapolation method..
...
```text
Not implemented yet. Suggested future file: src/methods/gragg_bulirsch_stoer.m
```
...
Gragg-Bulirsch-Stoer should be understood as part of the **Modified midpoint plus extrapolation method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Modified midpoint plus extrapolation method..
