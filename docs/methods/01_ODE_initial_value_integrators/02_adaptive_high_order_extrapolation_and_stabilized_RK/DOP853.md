...
High-order explicit Dormand-Prince 8(5,3) method..
...
```text
Not implemented yet. Suggested future file: src/methods/dop853.m
```
...
DOP853 should be understood as part of the **High-order explicit Dormand-Prince 8(5,3) method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into High-order explicit Dormand-Prince 8(5,3) method..
