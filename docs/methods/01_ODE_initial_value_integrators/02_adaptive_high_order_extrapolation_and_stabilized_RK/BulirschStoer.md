...
Extrapolation method for high-accuracy smooth ODEs..
...
```text
Not implemented yet. Suggested future file: src/methods/bulirsch_stoer.m
```
...
Bulirsch-Stoer should be understood as part of the **Extrapolation method for high-accuracy smooth ODEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Extrapolation method for high-accuracy smooth ODEs..
