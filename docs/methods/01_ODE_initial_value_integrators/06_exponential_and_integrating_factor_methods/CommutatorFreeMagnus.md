...
Magnus-type method avoiding explicit commutators..
...
```text
Not implemented yet. Suggested future file: src/methods/commutator_free_magnus.m
```
...
Commutator-Free Magnus should be understood as part of the **Magnus-type method avoiding explicit commutators.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Magnus-type method avoiding explicit commutators..
