# Rational Krylov Exponential Integrator

## Category

Exponential integrator using rational Krylov subspaces..
...
```text
Not implemented yet. Suggested future file: src/methods/rational_krylov_exponential_integrator.m
```
...
For a split semilinear problem

$$
y'=Ay+g(t,y),
$$

exponential methods treat the stiff linear part through matrix functions such as

$$
e^{hA}, \qquad \varphi_k(hA).
$$

### Core idea

Rational Krylov Exponential Integrator should be understood as part of the **Exponential integrator using rational Krylov subspaces.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Exponential integrator using rational Krylov subspaces..
...
semilinear stiff problems with a dominant linear operator
