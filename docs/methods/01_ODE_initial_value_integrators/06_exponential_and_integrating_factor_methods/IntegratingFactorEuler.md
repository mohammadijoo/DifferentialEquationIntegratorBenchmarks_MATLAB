...
Euler method applied after an integrating-factor transform..
...
```text
Not implemented yet. Suggested future file: src/methods/integrating_factor_euler.m
```
...
Integrating Factor Euler should be understood as part of the **Euler method applied after an integrating-factor transform.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Euler method applied after an integrating-factor transform..
