...
Exponential method for linear time-dependent systems..
...
```text
Not implemented yet. Suggested future file: src/methods/magnus_integrator.m
```
...
Magnus Integrator should be understood as part of the **Exponential method for linear time-dependent systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Exponential method for linear time-dependent systems..
