...
Second-order exponential one-step method..
...
```text
Not implemented yet. Suggested future file: src/methods/exponential_midpoint.m
```
...
Exponential Midpoint should be understood as part of the **Second-order exponential one-step method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Second-order exponential one-step method..
