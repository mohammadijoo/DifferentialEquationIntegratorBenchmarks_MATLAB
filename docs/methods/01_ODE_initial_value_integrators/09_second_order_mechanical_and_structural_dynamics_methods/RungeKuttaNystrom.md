...
Direct method family for second-order ODEs..
...
```text
Not implemented yet. Suggested future file: src/methods/runge_kutta_nystrom.m
```
...
Runge-Kutta-Nyström should be understood as part of the **Direct method family for second-order ODEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Direct method family for second-order ODEs..
