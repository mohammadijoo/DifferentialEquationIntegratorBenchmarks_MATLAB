...
Hilber-Hughes-Taylor alpha method with numerical damping..
...
```text
Not implemented yet. Suggested future file: src/methods/hhtalpha.m
```
...
HHT-alpha Method should be understood as part of the **Hilber-Hughes-Taylor alpha method with numerical damping.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Hilber-Hughes-Taylor alpha method with numerical damping..
