...
Partitioned implicit Runge-Kutta method paired with Lobatto IIIA..
...
```text
Not implemented yet. Suggested future file: src/methods/lobatto_iiib.m
```
...
Lobatto IIIB should be understood as part of the **Partitioned implicit Runge-Kutta method paired with Lobatto IIIA.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Partitioned implicit Runge-Kutta method paired with Lobatto IIIA..
