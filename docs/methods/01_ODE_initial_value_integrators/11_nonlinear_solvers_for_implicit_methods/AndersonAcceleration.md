...
Acceleration method for fixed-point iterations..
...
```text
Not implemented yet. Suggested future file: src/methods/anderson_acceleration.m
```
...
Anderson Acceleration should be understood as part of the **Acceleration method for fixed-point iterations.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Acceleration method for fixed-point iterations..
