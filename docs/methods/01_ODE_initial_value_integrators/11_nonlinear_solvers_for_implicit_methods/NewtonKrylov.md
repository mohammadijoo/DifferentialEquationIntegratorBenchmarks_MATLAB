...
Matrix-free Newton method using Krylov linear solves..
...
```text
Not implemented yet. Suggested future file: src/methods/newton_krylov.m
```
...
Newton-Krylov / JFNK should be understood as part of the **Matrix-free Newton method using Krylov linear solves.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Matrix-free Newton method using Krylov linear solves..
