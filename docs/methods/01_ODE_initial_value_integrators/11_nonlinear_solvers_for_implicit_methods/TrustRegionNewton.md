...
Newton method with trust-region globalization..
...
```text
Not implemented yet. Suggested future file: src/methods/trust_region_newton.m
```
...
Trust-Region Newton should be understood as part of the **Newton method with trust-region globalization.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Newton method with trust-region globalization..
