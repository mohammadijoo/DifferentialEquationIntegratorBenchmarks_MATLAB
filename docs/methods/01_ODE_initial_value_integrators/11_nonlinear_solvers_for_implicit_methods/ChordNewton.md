...
Newton iteration with a frozen Jacobian..
...
```text
Not implemented yet. Suggested future file: src/methods/chord_newton.m
```
...
Chord Newton should be understood as part of the **Newton iteration with a frozen Jacobian.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Newton iteration with a frozen Jacobian..
