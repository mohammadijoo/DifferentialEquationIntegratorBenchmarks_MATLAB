...
Simple nonlinear iteration for implicit equations..
...
```text
Not implemented yet. Suggested future file: src/methods/picard_iteration.m
```
...
Picard / Fixed-Point Iteration should be understood as part of the **Simple nonlinear iteration for implicit equations.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Simple nonlinear iteration for implicit equations..
