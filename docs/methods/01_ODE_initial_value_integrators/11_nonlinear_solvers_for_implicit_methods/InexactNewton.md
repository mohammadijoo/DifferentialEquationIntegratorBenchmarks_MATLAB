...
Newton method with approximate linear solves..
...
```text
Not implemented yet. Suggested future file: src/methods/inexact_newton.m
```
...
Inexact Newton should be understood as part of the **Newton method with approximate linear solves.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Newton method with approximate linear solves..
