...
Newton variant reusing Jacobians or factorizations..
...
```text
Not implemented yet. Suggested future file: src/methods/modified_newton.m
```
...
Modified Newton should be understood as part of the **Newton variant reusing Jacobians or factorizations.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Newton variant reusing Jacobians or factorizations..
