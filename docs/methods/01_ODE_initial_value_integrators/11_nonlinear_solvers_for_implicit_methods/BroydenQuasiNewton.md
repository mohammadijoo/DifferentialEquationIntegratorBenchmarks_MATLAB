...
Jacobian-updating nonlinear solver avoiding full Jacobian recomputation..
...
```text
Not implemented yet. Suggested future file: src/methods/broyden_quasi_newton.m
```
...
A typical nonlinear residual has the form

$$
G(x)=0.
$$

Newton-type methods build corrections from

$$
J(x^{(k)})\Delta x^{(k)}=-G(x^{(k)}), \qquad x^{(k+1)}=x^{(k)}+\alpha_k\Delta x^{(k)}.
$$

Fixed-point methods instead seek

$$
x=g(x), \qquad x^{(k+1)}=g(x^{(k)}).
$$

### Core idea

Broyden / Quasi-Newton should be understood as part of the **Jacobian-updating nonlinear solver avoiding full Jacobian recomputation.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Jacobian-updating nonlinear solver avoiding full Jacobian recomputation..
...
implicit integrators, DAEs, nonlinear steady states, and nonlinear PDE discretizations
