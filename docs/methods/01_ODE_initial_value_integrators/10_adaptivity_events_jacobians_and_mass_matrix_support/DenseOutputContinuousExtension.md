...
Interpolation formula giving solution values inside accepted steps..
...
```text
Not implemented yet. Suggested future file: src/methods/dense_output_continuous_extension.m
```
...
Dense Output / Continuous Extension should be understood as part of the **Interpolation formula giving solution values inside accepted steps.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Interpolation formula giving solution values inside accepted steps..
