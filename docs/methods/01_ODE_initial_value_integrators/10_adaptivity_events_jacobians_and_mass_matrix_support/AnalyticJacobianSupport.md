...
Using user-supplied exact Jacobians in implicit solvers..
...
```text
Not implemented yet. Suggested future file: src/methods/analytic_jacobian_support.m
```
...
Analytic Jacobian Support should be understood as part of the **Using user-supplied exact Jacobians in implicit solvers.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Using user-supplied exact Jacobians in implicit solvers..
