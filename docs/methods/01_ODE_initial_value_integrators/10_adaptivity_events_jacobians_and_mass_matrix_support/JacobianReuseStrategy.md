...
Policy for reusing Jacobians and matrix factorizations..
...
```text
Not implemented yet. Suggested future file: src/methods/jacobian_reuse_strategy.m
```
...
Jacobian Reuse Strategy should be understood as part of the **Policy for reusing Jacobians and matrix factorizations.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Policy for reusing Jacobians and matrix factorizations..
