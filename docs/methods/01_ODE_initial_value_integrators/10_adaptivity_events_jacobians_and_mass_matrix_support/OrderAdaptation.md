...
Method-order selection in variable-order solvers..
...
```text
Not implemented yet. Suggested future file: src/methods/order_adaptation.m
```
...
Order Adaptation should be understood as part of the **Method-order selection in variable-order solvers.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Method-order selection in variable-order solvers..
