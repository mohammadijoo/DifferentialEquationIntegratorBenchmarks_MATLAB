...
Sparse matrix handling for large stiff systems..
...
```text
Not implemented yet. Suggested future file: src/methods/sparse_jacobian_support.m
```
...
Sparse Jacobian Support should be understood as part of the **Sparse matrix handling for large stiff systems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Sparse matrix handling for large stiff systems..
