# Nonlinear Convergence Failure Handling

## Category

Rules for recovery when implicit nonlinear solves fail..
...
```text
Not implemented yet. Suggested future file: src/methods/nonlinear_convergence_failure_handling.m
```
...
Nonlinear Convergence Failure Handling should be understood as part of the **Rules for recovery when implicit nonlinear solves fail.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Rules for recovery when implicit nonlinear solves fail..
