...
Support for systems M(t,y)y_prime=f(t,y)..
...
```text
Not implemented yet. Suggested future file: src/methods/mass_matrix_support.m
```
...
Mass Matrix Support should be understood as part of the **Support for systems M(t,y)y_prime=f(t,y).** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Support for systems M(t,y)y_prime=f(t,y)..
