...
Implicit-explicit backward differentiation formulas..
...
```text
Not implemented yet. Suggested future file: src/methods/imexbdf.m
```
...
IMEX-BDF should be understood as part of the **Implicit-explicit backward differentiation formulas.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Implicit-explicit backward differentiation formulas..
