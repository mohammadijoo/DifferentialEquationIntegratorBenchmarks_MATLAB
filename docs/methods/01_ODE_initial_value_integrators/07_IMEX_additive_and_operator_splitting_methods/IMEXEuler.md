...
First-order implicit-explicit split method..
...
```text
Not implemented yet. Suggested future file: src/methods/imexeuler.m
```
...
IMEX Euler should be understood as part of the **First-order implicit-explicit split method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into First-order implicit-explicit split method..
