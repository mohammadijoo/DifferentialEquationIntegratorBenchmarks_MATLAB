...
Runge-Kutta methods with explicit and implicit partitions..
...
```text
Not implemented yet. Suggested future file: src/methods/imexrunge_kutta.m
```
...
IMEX Runge-Kutta should be understood as part of the **Runge-Kutta methods with explicit and implicit partitions.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Runge-Kutta methods with explicit and implicit partitions..
