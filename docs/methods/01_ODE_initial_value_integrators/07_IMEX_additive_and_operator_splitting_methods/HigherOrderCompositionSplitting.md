# Higher-Order Composition Splitting

## Category

High-order splitting from compositions of lower-order flows..
...
```text
Not implemented yet. Suggested future file: src/methods/higher_order_composition_splitting.m
```
...
Higher-Order Composition Splitting should be understood as part of the **High-order splitting from compositions of lower-order flows.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into High-order splitting from compositions of lower-order flows..
