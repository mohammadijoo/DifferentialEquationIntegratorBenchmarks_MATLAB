...
Split method advancing physical sub-processes separately..
...
```text
Not implemented yet. Suggested future file: src/methods/fractional_step_method.m
```
...
Fractional-Step Method should be understood as part of the **Split method advancing physical sub-processes separately.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Split method advancing physical sub-processes separately..
