...
Projection-like method for incompressible flow..
...
```text
Not implemented yet. Suggested future file: src/methods/pressure_correction_method.m
```
...
Pressure-Correction Method should be understood as part of the **Projection-like method for incompressible flow.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Projection-like method for incompressible flow..
