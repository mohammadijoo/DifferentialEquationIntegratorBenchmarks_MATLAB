...
```text
Not implemented yet. Suggested future file: src/methods/lie_trotter_splitting.m
```
...
Lie-Trotter Splitting should be understood as part of the **First-order operator splitting.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into First-order operator splitting..
