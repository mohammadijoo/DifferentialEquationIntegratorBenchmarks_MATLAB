# Higher-Order Yoshida Composition

## Category

Higher-order symplectic splitting compositions beyond fourth order..
...
```text
Not implemented yet. Suggested future file: src/methods/higher_order_yoshida_composition.m
```
...
For an additively split problem

$$
y'=f_E(t,y)+f_I(t,y),
$$

the nonstiff part $f_E$ can be handled explicitly while the stiff part $f_I$ is handled implicitly or by a separate exact/substep flow.

### Core idea

Higher-Order Yoshida Composition should be understood as part of the **Higher-order symplectic splitting compositions beyond fourth order.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Higher-order symplectic splitting compositions beyond fourth order..
...
systems with natural stiff/nonstiff or physics-based operator splitting
