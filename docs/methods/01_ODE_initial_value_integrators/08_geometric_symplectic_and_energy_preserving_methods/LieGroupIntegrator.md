...
Integrator for dynamics evolving on matrix Lie groups..
...
```text
Not implemented yet. Suggested future file: src/methods/lie_group_integrator.m
```
...
Lie Group Integrator should be understood as part of the **Integrator for dynamics evolving on matrix Lie groups.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Integrator for dynamics evolving on matrix Lie groups..
