...
Integrator derived from a discrete action principle..
...
```text
Not implemented yet. Suggested future file: src/methods/variational_integrator.m
```
...
Variational Integrator should be understood as part of the **Integrator derived from a discrete action principle.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Integrator derived from a discrete action principle..
