...
Runge-Kutta methods with separate updates for partitioned variables..
...
```text
Not implemented yet. Suggested future file: src/methods/partitioned_runge_kutta.m
```
...
Partitioned Runge-Kutta should be understood as part of the **Runge-Kutta methods with separate updates for partitioned variables.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Runge-Kutta methods with separate updates for partitioned variables..
