...
Variable-order BDF stiff solver style..
...
```text
Not implemented yet. Suggested future file: src/methods/cvodebdf.m
```
...
CVODE BDF should be understood as part of the **Variable-order BDF stiff solver style.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Variable-order BDF stiff solver style..
