...
Variable-order Adams-Moulton nonstiff solver style..
...
```text
Not implemented yet. Suggested future file: src/methods/cvodeadams.m
```
...
CVODE Adams should be understood as part of the **Variable-order Adams-Moulton nonstiff solver style.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Variable-order Adams-Moulton nonstiff solver style..
