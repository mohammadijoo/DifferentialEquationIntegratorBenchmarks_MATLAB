# Variable-Step Variable-Order NDF

## Category

ode15s-style NDF solver with adaptive order and step size..
...
```text
Not implemented yet. Suggested future file: src/methods/variable_step_variable_order_ndf.m
```
...
Variable-Step Variable-Order NDF should be understood as part of the **ode15s-style NDF solver with adaptive order and step size.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into ode15s-style NDF solver with adaptive order and step size..
