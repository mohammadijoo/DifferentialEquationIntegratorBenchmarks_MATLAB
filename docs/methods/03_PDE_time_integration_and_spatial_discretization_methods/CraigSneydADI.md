...
ADI method for convection-diffusion and finance PDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/craig_sneyd_adi.m
```
...
Craig-Sneyd ADI should be understood as part of the **ADI method for convection-diffusion and finance PDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into ADI method for convection-diffusion and finance PDEs..
