...
Backward-time centered-space implicit finite-difference scheme..
...
```text
Not implemented yet. Suggested future file: src/methods/btcs.m
```
...
BTCS should be understood as part of the **Backward-time centered-space implicit finite-difference scheme.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Backward-time centered-space implicit finite-difference scheme..
