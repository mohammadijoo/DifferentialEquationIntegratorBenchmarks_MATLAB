...
Predictor-corrector scheme for advection/flow problems..
...
```text
Not implemented yet. Suggested future file: src/methods/mac_cormack.m
```
...
MacCormack Method should be understood as part of the **Predictor-corrector scheme for advection/flow problems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Predictor-corrector scheme for advection/flow problems..
