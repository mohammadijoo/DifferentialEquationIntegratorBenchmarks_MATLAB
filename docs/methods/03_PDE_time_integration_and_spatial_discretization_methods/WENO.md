...
Weighted essentially non-oscillatory high-order method..
...
```text
Not implemented yet. Suggested future file: src/methods/weno.m
```
...
WENO should be understood as part of the **Weighted essentially non-oscillatory high-order method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Weighted essentially non-oscillatory high-order method..
