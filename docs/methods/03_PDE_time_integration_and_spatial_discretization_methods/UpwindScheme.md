...
Directionally biased discretization for advection..
...
```text
Not implemented yet. Suggested future file: src/methods/upwind_scheme.m
```
...
Upwind Scheme should be understood as part of the **Directionally biased discretization for advection.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Directionally biased discretization for advection..
