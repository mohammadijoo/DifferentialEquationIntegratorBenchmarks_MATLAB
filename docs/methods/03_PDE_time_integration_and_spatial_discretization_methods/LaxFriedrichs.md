...
Dissipative scheme for hyperbolic conservation laws..
...
```text
Not implemented yet. Suggested future file: src/methods/lax_friedrichs.m
```
...
Lax-Friedrichs should be understood as part of the **Dissipative scheme for hyperbolic conservation laws.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Dissipative scheme for hyperbolic conservation laws..
