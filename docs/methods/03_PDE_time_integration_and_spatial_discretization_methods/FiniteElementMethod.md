...
Weak-form spatial discretization over elements..
...
```text
Not implemented yet. Suggested future file: src/methods/finite_element_method.m
```
...
Finite Element Method should be understood as part of the **Weak-form spatial discretization over elements.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Weak-form spatial discretization over elements..
