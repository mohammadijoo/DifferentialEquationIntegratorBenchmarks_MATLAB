...
Implicit-explicit PDE time stepping..
...
```text
Not implemented yet. Suggested future file: src/methods/pdeimexscheme.m
```
...
PDE IMEX Scheme should be understood as part of the **Implicit-explicit PDE time stepping.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Implicit-explicit PDE time stepping..
