...
Trajectory-based method for advection and transport..
...
```text
Not implemented yet. Suggested future file: src/methods/semi_lagrangian_method.m
```
...
Semi-Lagrangian Method should be understood as part of the **Trajectory-based method for advection and transport.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Trajectory-based method for advection and transport..
