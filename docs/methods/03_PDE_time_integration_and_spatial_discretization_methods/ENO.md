...
Essentially non-oscillatory shock-capturing method..
...
```text
Not implemented yet. Suggested future file: src/methods/eno.m
```
...
ENO should be understood as part of the **Essentially non-oscillatory shock-capturing method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Essentially non-oscillatory shock-capturing method..
