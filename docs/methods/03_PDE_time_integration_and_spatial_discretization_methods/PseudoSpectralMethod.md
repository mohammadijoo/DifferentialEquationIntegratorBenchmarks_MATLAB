...
Spectral method evaluating nonlinearities in physical space..
...
```text
Not implemented yet. Suggested future file: src/methods/pseudo_spectral_method.m
```
...
Pseudo-Spectral Method should be understood as part of the **Spectral method evaluating nonlinearities in physical space.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Spectral method evaluating nonlinearities in physical space..
