...
Spatial discretization followed by ODE/DAE time integration..
...
```text
Not implemented yet. Suggested future file: src/methods/method_of_lines.m
```
...
Method of Lines should be understood as part of the **Spatial discretization followed by ODE/DAE time integration.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Spatial discretization followed by ODE/DAE time integration..
