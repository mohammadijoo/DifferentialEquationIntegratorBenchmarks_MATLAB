...
Arbitrary high-order derivative method for hyperbolic PDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/ader.m
```
...
ADER should be understood as part of the **Arbitrary high-order derivative method for hyperbolic PDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Arbitrary high-order derivative method for hyperbolic PDEs..
