...
Second-order implicit method for parabolic PDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/crank_nicolson_pde.m
```
...
Crank-Nicolson PDE Method should be understood as part of the **Second-order implicit method for parabolic PDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Second-order implicit method for parabolic PDEs..
