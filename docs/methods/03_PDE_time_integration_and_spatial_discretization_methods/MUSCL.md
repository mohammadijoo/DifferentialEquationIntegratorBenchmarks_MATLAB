...
High-resolution finite-volume reconstruction method..
...
```text
Not implemented yet. Suggested future file: src/methods/muscl.m
```
...
MUSCL should be understood as part of the **High-resolution finite-volume reconstruction method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into High-resolution finite-volume reconstruction method..
