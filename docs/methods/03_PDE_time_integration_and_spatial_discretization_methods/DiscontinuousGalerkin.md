...
Element-local polynomial method with numerical fluxes..
...
```text
Not implemented yet. Suggested future file: src/methods/discontinuous_galerkin.m
```
...
Discontinuous Galerkin Method should be understood as part of the **Element-local polynomial method with numerical fluxes.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Element-local polynomial method with numerical fluxes..
