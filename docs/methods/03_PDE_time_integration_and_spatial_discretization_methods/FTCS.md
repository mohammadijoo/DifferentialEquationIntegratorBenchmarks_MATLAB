...
```text
Not implemented yet. Suggested future file: src/methods/ftcs.m
```
...
FTCS should be understood as part of the **Forward-time centered-space finite-difference scheme.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Forward-time centered-space finite-difference scheme..
