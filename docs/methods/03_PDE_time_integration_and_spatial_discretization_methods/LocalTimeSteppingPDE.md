# Local Time Stepping for PDEs

## Category

Different time steps in different mesh regions..
...
```text
Not implemented yet. Suggested future file: src/methods/local_time_stepping_pde.m
```
...
Local Time Stepping for PDEs should be understood as part of the **Different time steps in different mesh regions.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Different time steps in different mesh regions..
