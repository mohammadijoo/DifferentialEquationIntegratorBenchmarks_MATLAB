...
Spectral method for periodic smooth PDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/fourier_spectral_method.m
```
...
Fourier Spectral Method should be understood as part of the **Spectral method for periodic smooth PDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Spectral method for periodic smooth PDEs..
