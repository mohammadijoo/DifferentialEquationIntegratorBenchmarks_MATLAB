...
Grid-based spatial discretization for PDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/finite_difference_method.m
```
...
Finite Difference Method should be understood as part of the **Grid-based spatial discretization for PDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Grid-based spatial discretization for PDEs..
