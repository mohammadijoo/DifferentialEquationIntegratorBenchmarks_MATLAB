...
Spectral collocation method on bounded intervals..
...
```text
Not implemented yet. Suggested future file: src/methods/chebyshev_spectral_collocation.m
```
...
Chebyshev Spectral Collocation should be understood as part of the **Spectral collocation method on bounded intervals.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Spectral collocation method on bounded intervals..
