...
Classical ADI method for multidimensional diffusion..
...
```text
Not implemented yet. Suggested future file: src/methods/peaceman_rachford_adi.m
```
...
Peaceman-Rachford ADI should be understood as part of the **Classical ADI method for multidimensional diffusion.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Classical ADI method for multidimensional diffusion..
