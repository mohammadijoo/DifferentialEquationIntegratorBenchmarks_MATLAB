# PDE Exponential Time Differencing

## Category

ETD time stepping for stiff semilinear PDEs..
...
```text
Not implemented yet. Suggested future file: src/methods/pdeexponential_time_differencing.m
```
...
A PDE time integrator is usually combined with a spatial discretization. The method of lines gives

$$
\frac{dU}{dt}=F(t,U),
$$

where $U$ contains nodal, modal, or cell-average unknowns. Time stepping is then applied to this large ODE or DAE system.

### Core idea

PDE Exponential Time Differencing should be understood as part of the **ETD time stepping for stiff semilinear PDEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
```text
construct spatial grid, mesh, cells, or basis functions
assemble the semi-discrete operator
advance the semi-discrete state in time
apply boundary conditions and stability checks
```

## Historical background

PDE Exponential Time Differencing belongs to a broader numerical-analysis literature on time integration, nonlinear equation solving, differential-algebraic equations, partial differential equations, stochastic models, or structure-preserving computation. The documentation here is intended as a practical engineering summary for benchmark planning.
...
- Extends the benchmark coverage into ETD time stepping for stiff semilinear PDEs..
...
stiff dissipative ODEs, chemical kinetics, circuit-like systems, and semi-discrete parabolic PDEs
