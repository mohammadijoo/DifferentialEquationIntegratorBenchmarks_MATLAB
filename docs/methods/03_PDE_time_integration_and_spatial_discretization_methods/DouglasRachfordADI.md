...
ADI method for multidimensional parabolic problems..
...
```text
Not implemented yet. Suggested future file: src/methods/douglas_rachford_adi.m
```
...
Douglas-Rachford ADI should be understood as part of the **ADI method for multidimensional parabolic problems.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into ADI method for multidimensional parabolic problems..
