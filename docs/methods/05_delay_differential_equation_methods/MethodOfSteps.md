...
Core framework for retarded delay differential equations..
...
```text
Not implemented yet. Suggested future file: src/methods/method_of_steps.m
```
...
Method of Steps should be understood as part of the **Core framework for retarded delay differential equations.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Core framework for retarded delay differential equations..
