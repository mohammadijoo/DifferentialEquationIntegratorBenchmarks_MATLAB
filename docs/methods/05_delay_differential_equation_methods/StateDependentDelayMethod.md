# State-Dependent Delay Method

## Category

DDE method for delays depending on the solution state..
...
```text
Not implemented yet. Suggested future file: src/methods/state_dependent_delay_method.m
```
...
State-Dependent Delay Method should be understood as part of the **DDE method for delays depending on the solution state.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into DDE method for delays depending on the solution state..
