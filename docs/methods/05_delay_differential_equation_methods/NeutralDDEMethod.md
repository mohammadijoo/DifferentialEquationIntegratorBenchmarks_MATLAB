...
Method for equations involving delayed derivatives..
...
```text
Not implemented yet. Suggested future file: src/methods/neutral_ddemethod.m
```
...
Neutral DDE Method should be understood as part of the **Method for equations involving delayed derivatives.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Method for equations involving delayed derivatives..
