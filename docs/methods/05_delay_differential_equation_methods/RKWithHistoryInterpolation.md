# RK with History Interpolation

## Category

Runge-Kutta DDE method using interpolated delayed states..
...
```text
Not implemented yet. Suggested future file: src/methods/rkwith_history_interpolation.m
```
...
A delay differential equation may have the form

$$
y'(t)=f(t,y(t),y(t-\tau)).
$$

The integrator needs a history function and interpolation of previously computed states when $t-\tau$ does not fall exactly on a time grid point.

### Core idea

RK with History Interpolation should be understood as part of the **Runge-Kutta DDE method using interpolated delayed states.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Runge-Kutta DDE method using interpolated delayed states..
