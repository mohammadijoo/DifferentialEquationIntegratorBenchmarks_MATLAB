# BDF/NDF with Mass Matrix

## Category

BDF/NDF methods for M(t,y)y_prime=f(t,y), including singular M..
...
```text
Not implemented yet. Suggested future file: src/methods/bdfwith_mass_matrix.m
```
...
BDF/NDF with Mass Matrix should be understood as part of the **BDF/NDF methods for M(t,y)y_prime=f(t,y), including singular M.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into BDF/NDF methods for M(t,y)y_prime=f(t,y), including singular M..
