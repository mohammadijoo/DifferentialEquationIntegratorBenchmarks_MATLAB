...
Position/velocity constraint method related to SHAKE..
...
```text
Not implemented yet. Suggested future file: src/methods/rattle.m
```
...
RATTLE should be understood as part of the **Position/velocity constraint method related to SHAKE.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Position/velocity constraint method related to SHAKE..
