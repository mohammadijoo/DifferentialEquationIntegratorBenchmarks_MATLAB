...
Index-reduction method using selected derivative substitutions..
...
```text
Not implemented yet. Suggested future file: src/methods/dummy_derivative_method.m
```
...
Dummy Derivative Method should be understood as part of the **Index-reduction method using selected derivative substitutions.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Index-reduction method using selected derivative substitutions..
