...
Position-constraint method for molecular and mechanical DAEs..
...
```text
Not implemented yet. Suggested future file: src/methods/shake.m
```
...
SHAKE should be understood as part of the **Position-constraint method for molecular and mechanical DAEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Position-constraint method for molecular and mechanical DAEs..
