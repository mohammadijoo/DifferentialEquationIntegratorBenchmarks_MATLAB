...
Classic variable-order BDF DAE solver..
...
```text
Not implemented yet. Suggested future file: src/methods/dassl.m
```
...
DASSL should be understood as part of the **Classic variable-order BDF DAE solver.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Classic variable-order BDF DAE solver..
