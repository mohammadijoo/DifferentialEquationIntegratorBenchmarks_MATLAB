...
First-order implicit DAE time step..
...
```text
Not implemented yet. Suggested future file: src/methods/implicit_euler_dae.m
```
...
Implicit Euler for DAEs should be understood as part of the **First-order implicit DAE time step.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into First-order implicit DAE time step..
