...
Variable-order BDF method for general DAEs F(t,y,y_prime)=0..
...
```text
Not implemented yet. Suggested future file: src/methods/idastyle_bdf.m
```
...
IDA/DASSL-Style BDF should be understood as part of the **Variable-order BDF method for general DAEs F(t,y,y_prime)=0.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Variable-order BDF method for general DAEs F(t,y,y_prime)=0..
