...
Large-scale DAE solver with Krylov support..
...
```text
Not implemented yet. Suggested future file: src/methods/daspk.m
```
...
DASPK should be understood as part of the **Large-scale DAE solver with Krylov support.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Large-scale DAE solver with Krylov support..
