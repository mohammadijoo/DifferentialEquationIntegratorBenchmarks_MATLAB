...
Constraint stabilization for mechanical DAEs..
...
```text
Not implemented yet. Suggested future file: src/methods/baumgarte_stabilization.m
```
...
Baumgarte Stabilization should be understood as part of the **Constraint stabilization for mechanical DAEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Constraint stabilization for mechanical DAEs..
