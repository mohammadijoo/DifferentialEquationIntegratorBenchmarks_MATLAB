...
Implicit collocation method for stiff DAEs..
...
```text
Not implemented yet. Suggested future file: src/methods/radau_iiafor_dae.m
```
...
Radau IIA for DAEs should be understood as part of the **Implicit collocation method for stiff DAEs.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Implicit collocation method for stiff DAEs..
