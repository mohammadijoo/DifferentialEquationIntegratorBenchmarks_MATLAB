# Compound-Fast Multirate Method

## Category

Multirate method with multiple fast components..
...
```text
Not implemented yet. Suggested future file: src/methods/compound_fast_multirate.m
```
...
Compound-Fast Multirate Method should be understood as part of the **Multirate method with multiple fast components.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Multirate method with multiple fast components..
