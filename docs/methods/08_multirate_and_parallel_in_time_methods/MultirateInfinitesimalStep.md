...
Multirate method where fast dynamics are integrated inside slow stages..
...
```text
Not implemented yet. Suggested future file: src/methods/multirate_infinitesimal_step.m
```
...
For a fast/slow split system

$$
y'=f_s(t,y)+f_f(t,y),
$$

multirate and parallel-in-time methods use different temporal resolutions, coarse/fine propagators, or time-domain decomposition to reduce cost.

### Core idea

Multirate Infinitesimal Step should be understood as part of the **Multirate method where fast dynamics are integrated inside slow stages.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Multirate method where fast dynamics are integrated inside slow stages..
...
systems with separated time scales or expensive coupled subsystems
