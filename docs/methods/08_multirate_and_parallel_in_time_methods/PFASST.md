...
```text
Not implemented yet. Suggested future file: src/methods/pfasst.m
```
...
PFASST should be understood as part of the **Parallel full approximation scheme in space and time.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Parallel full approximation scheme in space and time..
