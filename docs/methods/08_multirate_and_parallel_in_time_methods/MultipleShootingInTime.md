# Multiple Shooting in Time

## Category

Time-domain decomposition and matching method..
...
```text
Not implemented yet. Suggested future file: src/methods/multiple_shooting_in_time.m
```
...
Multiple Shooting in Time should be understood as part of the **Time-domain decomposition and matching method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Time-domain decomposition and matching method..
