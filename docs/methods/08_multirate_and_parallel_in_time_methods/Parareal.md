...
Parallel-in-time method using coarse/fine propagators..
...
```text
Not implemented yet. Suggested future file: src/methods/parareal.m
```
...
Parareal should be understood as part of the **Parallel-in-time method using coarse/fine propagators.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Parallel-in-time method using coarse/fine propagators..
