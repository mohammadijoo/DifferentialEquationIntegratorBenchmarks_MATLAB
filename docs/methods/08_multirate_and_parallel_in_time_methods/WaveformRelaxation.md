...
Subsystem time-domain iteration method..
...
```text
Not implemented yet. Suggested future file: src/methods/waveform_relaxation.m
```
...
Waveform Relaxation should be understood as part of the **Subsystem time-domain iteration method.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Subsystem time-domain iteration method..
