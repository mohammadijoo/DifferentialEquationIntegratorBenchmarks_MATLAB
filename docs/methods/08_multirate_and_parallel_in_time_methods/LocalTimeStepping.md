...
Using different step sizes in different regions/components..
...
```text
Not implemented yet. Suggested future file: src/methods/local_time_stepping.m
```
...
Local Time Stepping should be understood as part of the **Using different step sizes in different regions/components.** class. In a benchmark suite, its purpose is not only to produce a trajectory, but also to expose how this class behaves with respect to stability, accuracy, robustness, and computational work.
...
- Extends the benchmark coverage into Using different step sizes in different regions/components..
